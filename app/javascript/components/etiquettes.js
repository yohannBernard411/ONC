const print = () => {
  if (document.getElementById("make")){
    ...
    makeButton.addEventListener("click", (event) => {
      const my_window = window.open('', 'mywindow', 'status=1,width=1200,height=800');
      ...
      my_window.document.write('<div style="display: flex; flex-flow: row wrap; justify-content: space-between;">');
        allAdress.forEach((adress) => {
          my_window.document.write(`<div style="border: dashed 1px black; width: 350px; height: auto; padding: 10px; margin-bottom: 5px"><p>${adress.querySelector(".name").innerHTML}</p>`);
          ...
          my_window.document.write(`<p>${adress.querySelector(".code").innerHTML}</p></div>`);
        });
